...
<div class="wh-prof max-w-3xl" style="max-width: 48rem; margin: 0 auto;">
    ...
    <div class="bg-white border border-[#E5EAF3] p-6 mb-4" style="border-radius: var(--radius-card); margin-bottom: 1rem; background: white; border: 1px solid #E5EAF3; padding: 1.5rem;">
        ...
        <div class="flex items-center gap-4 mb-5 p-4 bg-alice" style="display: flex; align-items: center; gap: 1rem; padding: 1rem; background: var(--alice); border-radius: calc(var(--radius-btn) - 2px); margin-bottom: 1.25rem;">
            <div id="avatarInitial" class="w-14 h-14 rounded-full bg-gradient-to-br from-cyan-400 to-cyan-600 flex items-center justify-center text-white text-[20px] font-bold flex-shrink-0" style="width: 3.5rem; height: 3.5rem; border-radius: 50%; background: linear-gradient(135deg, #22d3ee, #0891b2); display: flex; align-items: center; justify-content: center; color: white; font-size: 20px; font-weight: 700; flex-shrink: 0;">
            </div>
            <div class="flex-1" style="flex: 1;">
                ...
                <div id="avatarRole" class="text-navy/50 text-[12px]" style="color: rgba(16,55,92,0.50); font-size: 12px;">Warehouse Staff</div>
            </div>
        </div>

        <form id="whPersonalInfoForm" class="space-y-4" style="display: flex; flex-direction: column; gap: 1rem;">
            <div>
            ...
            <div class="grid grid-cols-2 gap-4" style="display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 1rem;">
                ...
                <div>
                    <label class="block text-navy/70 text-[12px] font-semibold mb-1.5" style="display: block; color: rgba(16,55,92,0.70); font-size: 12px; font-weight: 600; margin-bottom: 0.375rem;">Khu vực kho phụ trách</label>
                    <div class="relative" style="position: relative;">
                        <svg class="absolute left-3 top-1/2 -translate-y-1/2 text-navy/30" style="position: absolute; left: 0.75rem; top: 50%; transform: translateY(-50%); width: 16px; height: 16px; color: rgba(16,55,92,0.30);" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <input
                            ...
                            id="warehouseZone"
                            name="warehouseZone"
                            placeholder="Chưa cập nhật khu vực"
                            class="w-full pl-10 pr-4 py-2.5 bg-alice border border-[#E5EAF3] text-navy text-[13px] outline-none focus:border-navy/30 transition"
                            style="width: 100%; padding: 0.625rem 1rem 0.625rem 2.5rem; background: var(--alice); border: 1px solid #E5EAF3; color: var(--navy); font-size: 13px; outline: none; border-radius: calc(var(--radius-btn) - 2px); transition: border-color 0.2s;"
                        />
                    </div>
    ...
    <div class="bg-white border border-[#E5EAF3] p-6" style="border-radius: var(--radius-card); background: white; border: 1px solid #E5EAF3; padding: 1.5rem;">
        <div class="flex items-center gap-3 mb-5" style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1.25rem;">
            <div class="w-10 h-10 bg-alice flex items-center justify-center" style="width: 2.5rem; height: 2.5rem; background: var(--alice); display: flex; align-items: center; justify-content: center; border-radius: var(--radius-btn);">
                <svg class="text-navy/60" style="width: 20px; height: 20px; color: rgba(16,55,92,0.60);" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <div>
                ...
                <p class="text-navy/45 text-[12px]" style="color: rgba(16,55,92,0.45); font-size: 12px; margin: 0.125rem 0 0 0;">Đảm bảo tài khoản luôn được bảo mật</p>
            </div>
        </div>

        <form id="whPasswordForm" class="space-y-4" style="display: flex; flex-direction: column; gap: 1rem;">
            <div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Load initial values from localStorage (only for warehouseZone which is not in users DB schema)
    ...
    document.getElementById("saveProfileBtn").addEventListener("click", function () {
        ...
        const warehouseZone = document.getElementById("warehouseZone").value.trim();
        ...
        fetch("${pageContext.request.contextPath}/warehouse/profile", {
            method: "POST",
            headers: {
        ...
        .then(data => {
            if (data.success) {
                // Save zone to localStorage as it is not in the database table
                const profileObj = { fullName, email, phone, warehouseZone };
    ...
    document.getElementById("savePasswordBtn").addEventListener("click", function () {
        ...
        if (!curPass || !newPass || !confPass) {
            showToast("Vui lòng điền đầy đủ các thông tin mật khẩu", true);
            return;
        }
        ...
        // Checklist check (contains uppercase, lowercase, and number)
