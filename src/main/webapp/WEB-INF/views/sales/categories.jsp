...
<%-- ══ PANEL: ÁNH XẠ WMS ↔ LAZADA (UC-B2C09) ════════════════════════════ --%>
<div class="cat-layout-grid" style="margin-top:1.5rem">
    <div class="cat-panel-full">
        <div class="panel-hdr">
            <div>
                <h3 class="panel-title">
                    ...
                    Ánh xạ danh mục WMS ↔ Lazada
                </h3>
                <div style="font-size:12px;color:rgba(16,55,92,.5);margin-top:0.25rem">
                    Chọn 1 danhụ WMS → gán các leaf category Lazada tương ứng.
                    Khi push sản phẩm, wizard sẽ tự gợi ý leaf "primary" này.
                </div>
            </div>
        </div>

        <div style="display:grid; grid-template-columns: 1fr 2fr; gap:1rem; padding:1rem">
            <%-- LEFT: chọn WMS category --%>
            <div>
                <label class="cp-form-label">Chọn danh mục WMS</label>
                <select id="mappingWmsCategory" class="cp-input-text" style="width:100%;padding:0.5rem"
                        onchange="loadMappingsForWms()">
                    <option value="">-- Chọn danh mục --</option>
                </select>
                <div style="margin-top:0.75rem; font-size:11px; color:rgba(16,55,92,.5)">
                    <b>Primary</b> = leaf được ưu tiên khi push sản phẩm thuộc WMS category này.
                </div>
            </div>

            <%-- RIGHT: danh sách mappings --%>
            <div>
                <div id="mappingsList" style="border:1px solid #E5EAF3; border-radius:8px; min-height:160px; padding:0.75rem; background:#FAFBFC">
                    <div style="text-align:center;color:rgba(16,55,92,.4);padding:2rem">
                        Chọn 1 WMS category để xem mappings.
                    </div>
                </div>

                <div style="margin-top:0.75rem; display:grid; grid-template-columns: 1fr 2fr auto auto; gap:0.5rem">
                    <input type="number" id="newLazadaId" class="cp-input-text" style="padding:0.5rem" placeholder="Lazada Category ID" />
                    <input type="text" id="newLazadaName" class="cp-input-text" style="padding:0.5rem" placeholder="Tên (hiển thị)" />
                    <label style="display:flex;align-items:center;gap:0.25rem;font-size:12px">
                        <input type="checkbox" id="newIsPrimary" /> Primary
                    </label>
                    <button class="btn-primary-sm" onclick="addMapping()">+ Thêm</button>
                </div>
                <div style="margin-top:0.5rem; font-size:11px; color:rgba(16,55,92,.5)">
                    Gợi ý: bấm "Đồng bộ từ Lazada" ở wizard Push Product trước, sau đó copy ID từ dropdown về đây.
                </div>
            </div>
        </div>
    </div>
...
(function () {
    ...
    /* ─── WMS ↔ Lazada mapping (UC-B2C09) ─── */
    function populateWmsCategorySelect() {
        var sel = document.getElementById('mappingWmsCategory');
        if (!sel) return;
        var opts = '<option value="">-- Chọn danh mục --</option>';
        function recurse(parentId, prefix) {
            serverCategories.filter(function (c) {
                var p = c.parentId;
                return (parentId === null) ? (p === null || p === 0) : (p == parentId);
            }).forEach(function (node) {
                var name = (prefix || '') + (node.categoryName || node.name || '');
                opts += '<option value="' + node.categoryId + '">' + escapeHtml(name) + '</option>';
    ...
    window.loadMappingsForWms = function () {
        var wmsId = document.getElementById('mappingWmsCategory').value;
        if (!wmsId) {
            document.getElementById('mappingsList').innerHTML =
                '<div style="text-align:center;color:rgba(16,55,92,.4);padding:2rem">Chọn 1 WMS category để xem mappings.</div>';
            return;
        }
        // Lazada channel id = first Lazada channel. For now hardcode channelId=1.
        var channelId = 1;
        fetch('${pageContext.request.contextPath}/sales/category-mappings?action=load&channelId='
                + channelId + '&wmsCategoryId=' + wmsId)
            .then(function (r) { return r.json(); })
            .then(function (data) {
                if (!data.success) { showToast('Lỗi tải mapping: ' + data.message, 'error'); return; }
                renderMappings(data.mappings, wmsId, channelId);
            })
            .catch(function (e) { showToast('Lỗi: ' + e, 'error'); });
    };

    function renderMappings(mappings, wmsId, channelId) {
        var html = '';
        if (!mappings || mappings.length === 0) {
            html = '<div style="text-align:center;color:rgba(16,55,92,.4);padding:2rem">Chưa có mapping. Thêm ở form dưới.</div>';
        } else {
            html = '<table style="width:100%;border-collapse:collapse">';
            html += '<tr style="background:#F0F5FF;font-weight:700">'
                 + '<td style="padding:0.5rem">Lazada ID</td>'
                 + '<td style="padding:0.5rem">Tên</td>'
                 + '<td style="padding:0.5rem;text-align:center">Primary</td>'
                 + '<td style="padding:0.5rem"></td></tr>';
            mappings.forEach(function (m) {
                var star = m.isPrimary ? '★' : '☆';
                var color = m.isPrimary ? '#F59E0B' : '#999';
                html += '<tr style="border-top:1px solid #E5EAF3">'
                     + '<td style="padding:0.5rem;font-family:monospace">' + m.lazadaCategoryId + '</td>'
                     + '<td style="padding:0.5rem">' + escapeHtml(m.lazadaName || '(no name)') + '</td>'
                     + '<td style="padding:0.5rem;text-align:center;color:' + color
                     + ';font-size:18px;cursor:pointer" onclick="setPrimary(' + m.mappingId + ',' + channelId + ',' + wmsId + ')">'
                     + star + '</td>'
                     + '<td style="padding:0.5rem;text-align:right">'
                     + '<button class="cp-btn-edit" style="font-size:11px;padding:0.25rem 0.5rem" onclick="deleteMapping(' + m.mappingId + ',' + wmsId + ')">Xóa</button>'
                     + '</td></tr>';
            });
            html += '</table>';
        }
        document.getElementById('mappingsList').innerHTML = html;
    }

    window.addMapping = function () {
        var wmsId = document.getElementById('mappingWmsCategory').value;
        if (!wmsId) { showToast('Chọn WMS category trước', 'error'); return; }
        var lzId = document.getElementById('newLazadaId').value.trim();
        var lzName = document.getElementById('newLazadaName').value.trim();
        var isPrimary = document.getElementById('newIsPrimary').checked;
        if (!lzId) { showToast('Nhập Lazada Category ID', 'error'); return; }
        var params = new URLSearchParams();
        params.append('action', 'add');
        params.append('channelId', '1');
        params.append('wmsCategoryId', wmsId);
        params.append('lazadaCategoryId', lzId);
        params.append('lazadaName', lzName || ('ID=' + lzId));
        params.append('isPrimary', isPrimary ? '1' : '0');
        fetch('${pageContext.request.contextPath}/sales/category-mappings', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: params.toString()
        })
        ...
        .then(function (data) {
            if (data.success) {
                ...
                document.getElementById('newLazadaId').value = '';
                document.getElementById('newLazadaName').value = '';
    ...
    window.deleteMapping = function (mappingId, wmsId) {
        ...
        .then(function (r) { return r.json(); })
        .then(function (data) {
            if (data.success) { showToast('Đã xóa', 'success'); loadMappingsForWms(); }
            else { showToast('Lỗi: ' + data.message, 'error'); }
        });
    };

    window.setPrimary = function (mappingId, channelId, wmsId) {
        var params = new URLSearchParams();
        params.append('action', 'setPrimary');
        params.append('mappingId', mappingId);
        ...
        .then(function (r) { return r.json(); })
        .then(function (data) {
            if (data.success) { showToast('Đã đặt primary', 'success'); loadMappingsForWms(); }
            else { showToast('Lỗi: ' + data.message, 'error'); }
        });
    };
