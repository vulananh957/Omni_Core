...
<div class="users-management-container" style="max-width: 76rem; margin: 0 auto; padding-bottom: 2rem; position: relative;">
    ...
    <div id="statusToast" style="position: fixed; top: 1.5rem; right: 1.5rem; display: none; align-items: center; gap: 0.75rem; padding: 1rem 1.25rem; background: white; border-radius: var(--radius-btn); box-shadow: 0 10px 25px rgba(16, 55, 92, 0.15); z-index: 1000; border: 1px solid #10b981; border-left: 4px solid #10b981; transition: all 0.4s cubic-bezier(0.16, 1, 0.3, 1); opacity: 0; transform: translateY(-20px);">
        <span style="font-weight: bold; width: 1.5rem; height: 1.5rem; border-radius: 50%; display: flex; align-items: center; justify-content: center; color: white; background: #10b981; font-size: 12px;">✓</span>
        <span id="statusToastMsg" style="color: var(--navy); font-size: 13px; font-weight: 600;">Thành công!</span>
    </div>
        ...
        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
            ...
            <div style="display: flex; gap: 0.75rem; flex-wrap: wrap; flex: 1; align-items: center;">
                ...
                <div class="search-box" style="position: relative; width: 16rem;">
                    <svg class="search-box__icon" style="position: absolute; left: 0.75rem; top: 50%; transform: translateY(-50%); width: 15px; height: 15px; color: rgba(16, 55, 92, 0.35);" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <tbody>
                    <c:forEach var="user" items="${usersList}" varStatus="status">
                        <tr class="user-row" 
                            ...
                            <!-- STT -->
                            <td style="font-weight: 600; color: rgba(16, 55, 92, 0.60);">${status.index + 1}</td>
                            
                            <!-- Username -->
                            <td style="font-weight: 700; color: var(--navy);"><c:out value="${user.username}"/></td>
                            
                            <!-- Full Name -->
                            <td style="font-weight: 600; color: var(--navy);"><c:out value="${user.fullName}"/></td>
                            
                            <!-- Contact info -->
                            <td>
                                <div style="font-size: 13px; color: var(--navy); font-weight: 500;"><c:out value="${user.email}"/></div>
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${user.role == 'ADMIN'}">
                                        <span style="background: rgba(139, 92, 246, 0.1); color: #8b5cf6; padding: 0.25rem 0.5rem; font-size: 10px; font-weight: 800; border-radius: 4px; border: 1px solid rgba(139, 92, 246, 0.2); letter-spacing: 0.02em;">
                                            ADMIN
                                        </span>
                                    </c:when>
                                    <c:when test="${user.role == 'MANAGER'}">
                                        <span style="background: rgba(245, 200, 66, 0.1); color: #b58d05; padding: 0.25rem 0.5rem; font-size: 10px; font-weight: 800; border-radius: 4px; border: 1px solid rgba(245, 200, 66, 0.2); letter-spacing: 0.02em;">
                                            MANAGER
                                        </span>
                                    </c:when>
                                    <c:when test="${user.role == 'WAREHOUSE_STAFF'}">
                                        <span style="background: rgba(16, 115, 230, 0.1); color: #1073e6; padding: 0.25rem 0.5rem; font-size: 10px; font-weight: 800; border-radius: 4px; border: 1px solid rgba(16, 115, 230, 0.2); letter-spacing: 0.02em;">
                                            WAREHOUSE_STAFF
                                        </span>
                                    </c:when>
                                    <c:when test="${user.role == 'SALES_STAFF'}">
                                        <span style="background: rgba(235, 131, 23, 0.1); color: var(--orange); padding: 0.25rem 0.5rem; font-size: 10px; font-weight: 800; border-radius: 4px; border: 1px solid rgba(235, 131, 23, 0.2); letter-spacing: 0.02em;">
                                            SALES_STAFF
                                        </span>
                                    </c:when>
                                    <c:otherwise>
                                        <span style="background: rgba(16, 55, 92, 0.08); color: rgba(16, 55, 92, 0.60); padding: 0.25rem 0.5rem; font-size: 10px; font-weight: 800; border-radius: 4px; border: 1px solid rgba(16, 55, 92, 0.15); letter-spacing: 0.02em;">
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${user.active}">
                                        <span style="display: inline-flex; align-items: center; gap: 0.25rem; padding: 0.125rem 0.5rem; background: #e6f7ed; color: #10b981; font-size: 11px; font-weight: 700; border-radius: 20px; border: 1px solid rgba(16, 185, 129, 0.2);">
                                            Hoạt động
                                        </span>
                                    </c:when>
                                    <c:otherwise>
                                        <span style="display: inline-flex; align-items: center; gap: 0.25rem; padding: 0.125rem 0.5rem; background: #fee2e2; color: #ef4444; font-size: 11px; font-weight: 700; border-radius: 20px; border: 1px solid rgba(239, 68, 68, 0.2);">
                                            Đã khóa
                                        </span>
                                    </c:otherwise>
                                </c:choose>
                                ...
                                <div style="display: inline-flex; align-items: center; gap: 0.5rem; justify-content: flex-end;">
                                    ...
                                    <c:choose>
                                        <c:when test="${user.userId == loggedInUser.userId}">
                                            <a href="${pageContext.request.contextPath}/admin/profile" class="btn-outline" title="Cài đặt tài khoản của chính bạn"
                                        ...
                                        <c:otherwise>
                                            <a href="${pageContext.request.contextPath}/admin/users/edit?id=${user.userId}" class="btn-outline" title="Chỉnh sửa thông tin"
                                    ...
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <c:choose>
                                                <c:when test="${user.active}">
                                                    <a href="${pageContext.request.contextPath}/admin/users/toggle?id=${user.userId}&active=false" title="Vô hiệu hóa tài khoản"
                                                ...
                                                <c:otherwise>
                                                    <a href="${pageContext.request.contextPath}/admin/users/toggle?id=${user.userId}&active=true" title="Kích hoạt tài khoản"
                                                       style="text-decoration: none; padding: 0.5rem; font-size: 11px; border-radius: 6px; display: inline-flex; align-items: center; justify-content: center; background: #e6f7ed; color: #10b981; border: 1px solid rgba(16, 185, 129, 0.2); transition: all 0.15s ease;">
                                                        <svg style="width: 14px; height: 14px;" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                            <rect x="3" y="11" width="18" height="11" rx="2" ry="2" />
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="M8 11V7a4 4 0 118 0m-4 4v2" />
                                                        </svg>
                                                    </a>
                                                </c:otherwise>
                                            </c:choose>
                    ...
                    <c:if test="${empty usersList}">
                        <tr>
                            <td colspan="7" style="text-align: center; padding: 3rem; color: rgba(16, 55, 92, 0.5);">
